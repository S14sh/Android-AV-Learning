...
public:
    ...
    ADTSHead setSyncWord(const unsigned int &value = 0xfff);

    //关键字ID，MPEG Version: 0 for MPEG-4, 1 for MPEG-2,长度为1bit
    ADTSHead setID(const unsigned int &value = 0);

    //关键字layer,长度2bit,总是00
    ADTSHead setLayer(const unsigned int &value = 0);

    //关键字protection_absent,长度1bit(set to 1 if there is no CRC(循环冗余校验) and 0 if there is CRC)
    ADTSHead setProtectionAbsent(const unsigned int &value = 1);

    //关键字profile,长度2bit,表示使用哪个级别的AAC
    ADTSHead setProfile(const unsigned int &value = 1);
     ...
     */
    ADTSHead setSamplingFrequencyIndex(const unsigned int &value = 0x4);

    //关键字private_bit，长度1bit，私有位，编码时设置为0，解码时忽略
    ADTSHead setPrivateBit(const unsigned int &value = 0x1);
...
        8-15: Reserved
     */
    ADTSHead setChannelConfiguration(const unsigned int &value = 0x1);

    //关键字original_copy,长度1bit,编码时设置为0,解码时忽略
    ADTSHead setOriginalCopy(const unsigned int &value = 0);

    //关键字home,长度1bit,编码时设置为0,解码时忽略
    ADTSHead setHome(const unsigned int &value = 0);

    //关键字copyright_identification_bit,长度1bit,编码时设置为0,解码时忽略
    ADTSHead setCopyrightIdentificationBit(const unsigned int &value = 0);

    //关键字copyright_identification_start,长度1bit,编码时设置为0,解码时忽略
    ADTSHead setCopyrightIdentificationStart(const unsigned int &value = 0);

    //关键字aac_frame_length,长度13bit,一个ADTS帧的长度(包括ADTS头和AAC原始流)
    ADTSHead setAacFrameLength(const unsigned int &value = 0);

    //关键字adts_buffer_fullness,长度11bit,默认值0x7FF说明是码率可变的码流
    ADTSHead setAdtsBufferFullness(const unsigned int &value = 0x7ff);
    ...
    ADTSHead setNumberOfRawDataBlocksInFrame(const unsigned int &value = 0);
